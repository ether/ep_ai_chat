# Contributing to ep_ai_chat

Thanks for helping improve `ep_ai_chat` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_ai_chat` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
Edits in `../ep_ai_chat/` are picked up after a server restart.
